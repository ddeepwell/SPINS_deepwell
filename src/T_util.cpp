...
namespace TArrayn {
    ...
    void write_array_old(Array<double, 3> const & ar_d, const string basename, int seq_num, MPI_Comm c) {
        ofstream file;
    ...
    void write_array(TArrayn::DTArray & ar, char * basename,
        ...
        // Determine size_x, size_y, size_z
        // Yes, they could be passed as arguments,
        // but for the sake of compatability, compute them.
        int lb, ub, Nx, Ny, Nz;
        ub = ar.ubound(firstDim);
        lb = ar.lbound(firstDim);
        Nx = (ub - lb + 1)*numproc;

        ub = ar.ubound(secondDim);
        lb = ar.lbound(secondDim);
        Nz = ub - lb + 1;

        ub = ar.ubound(thirdDim);
        lb = ar.lbound(thirdDim);
        Ny = ub - lb + 1;

        // Determine the name for the created file
        char filename[80] = {0};
        strcat(filename, basename);
        if (seq_num >= 0) { 
            char conv[80]; 
            sprintf(conv, ".%d", seq_num);
            strcat(filename, conv);
        }

        count = Nx*Ny*Nz/numproc;
        offset = count*myrank;
        double *disk_data = new double[count];

        // Open the file
        fh_status = MPI_File_open(c, filename, MPI_MODE_WRONLY | MPI_MODE_CREATE, MPI_INFO_NULL, &fh);

        for (int II = 0; II < Nx*Ny*Nz/numproc; II++) {
            convert_index_2(offset + II, Nx, Ny, Nz, &Ix, &Iy, &Iz);
            disk_data[II] = ar(Ix,Iy,Iz);
        }

        MPI_File_seek(fh, offset*sizeof(double), MPI_SEEK_SET);
        MPI_File_write(fh, disk_data, count, MPI_DOUBLE, &status);
    ...
    void initialize_chain_tmps(char* varname, MPI_File ** files,
            double *** chain_coords, int * num_chains, MPI_Comm c) {

        files[0] = new MPI_File[num_chains[0]];
        files[1] = new MPI_File[num_chains[1]];
        files[2] = new MPI_File[num_chains[2]];

        char chain_buffer[80] = {0};
        char conv[80] = {0};
        int Ix;

        // Initialize x chain files
        for (int II = 0; II < num_chains[0]; II++) {
            chain_buffer[0] = '\0';
            conv[0] = '\0';
            strcat(chain_buffer, varname);
            sprintf(conv, "_chain_x_%d_tmp.bin",II);
            strcat(chain_buffer, conv);
            MPI_File_open(c, chain_buffer, MPI_MODE_RDWR | MPI_MODE_CREATE | MPI_MODE_DELETE_ON_CLOSE, MPI_INFO_NULL, &files[0][II]);
        }

        // Initialize y chain files
        for (int II = 0; II < num_chains[1]; II++) {
            chain_buffer[0] = '\0';
            conv[0] = '\0';
            strcat(chain_buffer, varname);
            sprintf(conv, "_chain_y_%d_tmp.bin",II);
            strcat(chain_buffer, conv);
            MPI_File_open(c, chain_buffer, MPI_MODE_RDWR | MPI_MODE_CREATE | MPI_MODE_DELETE_ON_CLOSE, MPI_INFO_NULL, &files[1][II]);
        }

        // Initialize z chain files
        for (int II = 0; II < num_chains[2]; II++) {
            chain_buffer[0] = '\0';
            conv[0] = '\0';
            strcat(chain_buffer, varname);
            sprintf(conv, "_chain_z_%d_tmp.bin",II);
            strcat(chain_buffer, conv);
            MPI_File_open(c, chain_buffer, MPI_MODE_RDWR | MPI_MODE_CREATE | MPI_MODE_DELETE_ON_CLOSE, MPI_INFO_NULL, &files[2][II]);
        }
    ...
    void initialize_chain_finals(char* varname, MPI_File ** files,
        ...
        for (int II = 0; II < num_chains[0]; II++) {
            ...
            conv[0] = '\0';
            strcat(chain_buffer, varname);
            sprintf(conv, "_chain_x_%d.bin",II);
            strcat(chain_buffer, conv);
            MPI_File_open(c, chain_buffer, MPI_MODE_WRONLY | MPI_MODE_CREATE, MPI_INFO_NULL, &files[0][II]);
        }
        ...
        for (int II = 0; II < num_chains[1]; II++) {
            ...
            conv[0] = '\0';
            strcat(chain_buffer, varname);
            sprintf(conv, "_chain_y_%d.bin",II);
            strcat(chain_buffer, conv);
            MPI_File_open(c, chain_buffer, MPI_MODE_WRONLY | MPI_MODE_CREATE, MPI_INFO_NULL, &files[1][II]);
        }
        ...
        for (int II = 0; II < num_chains[2]; II++) {
            ...
            conv[0] = '\0';
            strcat(chain_buffer, varname);
            sprintf(conv, "_chain_z_%d.bin",II);
            strcat(chain_buffer, conv);
            MPI_File_open(c, chain_buffer, MPI_MODE_WRONLY | MPI_MODE_CREATE, MPI_INFO_NULL, &files[2][II]);
        }
    }
    ...
    void write_chains_2(DTArray & var, MPI_File ** files, 
            ...
            double * z_chain_data_buffer, MPI_Comm c){

        int Ix, Iy, Iz, II, cnt;
        int offset, myrank, numproc;
        MPI_Status status;
        ...
        // x chains
        offset = Nx*chain_count + myrank*Nx/numproc;
        for (II = 0; II < num_chains[0]; II++) {

            Iy = (int)(Ny*chain_coords[0][0][II]);
            Iz = (int)(Nz*chain_coords[0][1][II]);

            cnt = 0;
            for (Ix = var.lbound(firstDim); Ix <= var.ubound(firstDim); Ix++) {
                x_chain_data_buffer[cnt] = var(Ix,Iy,Iz);
                cnt++;
            }

            MPI_File_seek(files[0][II], offset*sizeof(double), MPI_SEEK_SET);
            MPI_File_write(files[0][II], x_chain_data_buffer, Nx/numproc, MPI_DOUBLE, &status);
        }

        // y chains
        offset = Ny*chain_count;
        for (II = 0; II < num_chains[1]; II++) {

            Ix = (int)(Nx*chain_coords[1][0][II]);
            Iz = (int)(Nz*chain_coords[1][0][II]);

            if (Ix >= var.lbound(firstDim) && Ix <= var.ubound(firstDim)) {
                for (Iy = 0; Iy < Ny; Iy++) { y_chain_data_buffer[Iy] = var(Ix,Iy,Iz);  }
                MPI_File_seek(files[1][II], offset*sizeof(double), MPI_SEEK_SET);
                MPI_File_write(files[1][II], y_chain_data_buffer, Ny, MPI_DOUBLE, &status);
            }
        }

        // z chains
        offset = Nz*chain_count;
        for (II = 0; II < num_chains[2]; II++) {

            Ix = (int)(Nx*chain_coords[2][0][II]);
            Iy = (int)(Ny*chain_coords[2][1][II]);

            if (Ix >= var.lbound(firstDim) && Ix <= var.ubound(firstDim)) {
                for (Iz = 0; Iz < Nz; Iz++) { z_chain_data_buffer[Iz] = var(Ix,Iy,Iz);  }
                MPI_File_seek(files[2][II], offset*sizeof(double), MPI_SEEK_SET);
                MPI_File_write(files[2][II], z_chain_data_buffer, Nz, MPI_DOUBLE, &status);
            }
        }
    }

    void initialize_slice_tmps(char* varname, MPI_File ** files,
            double ** slice_coords, int * num_slices, MPI_Comm c) {

        files[0] = new MPI_File[num_slices[0]];
        files[1] = new MPI_File[num_slices[1]];
        files[2] = new MPI_File[num_slices[2]];

        char buffer[80] = {0};
        char conv[80]   = {0};
        int Ix;

        // Initialize yz slice files
        for (int II = 0; II < num_slices[0]; II++) {
            buffer[0] = '\0';
            conv[0] = '\0';
            strcat(buffer, varname);
            sprintf(conv, "_slice_yz_%d_tmp.bin", II);
            strcat(buffer, conv);
            MPI_File_open(c, buffer, MPI_MODE_RDWR | MPI_MODE_CREATE | MPI_MODE_DELETE_ON_CLOSE, MPI_INFO_NULL, &files[0][II]);
        }

        // Initialize xz slice files
        for (int II = 0; II < num_slices[1]; II++) {
            buffer[0] = '\0';
            conv[0] = '\0';
            strcat(buffer, varname);
            sprintf(conv, "_slice_xz_%d_tmp.bin", II);
            strcat(buffer, conv);
            MPI_File_open(c, buffer, MPI_MODE_RDWR | MPI_MODE_CREATE | MPI_MODE_DELETE_ON_CLOSE, MPI_INFO_NULL, &files[1][II]);
        }

        // Initialize xy slice files
        for (int II = 0; II < num_slices[2]; II++) {
            buffer[0] = '\0';
            conv[0] = '\0';
            strcat(buffer, varname);
            sprintf(conv, "_slice_xy_%d_tmp.bin", II);
            strcat(buffer, conv);
            MPI_File_open(c, buffer, MPI_MODE_RDWR | MPI_MODE_CREATE | MPI_MODE_DELETE_ON_CLOSE, MPI_INFO_NULL, &files[2][II]);
        }
    }
    ...
    void initialize_slice_finals(char* varname, MPI_File ** files,
        ...
        for (int II = 0; II < num_slices[0]; II++) {
            ...
            conv[0] = '\0';
            strcat(buffer, varname);
            sprintf(conv, "_slice_yz_%d.bin", II);
            strcat(buffer, conv);
            MPI_File_open(c, buffer, MPI_MODE_WRONLY | MPI_MODE_CREATE, MPI_INFO_NULL, &files[0][II]);
        }
        ...
        for (int II = 0; II < num_slices[1]; II++) {
            ...
            conv[0] = '\0';
            strcat(buffer, varname);
            sprintf(conv, "_slice_xz_%d.bin", II);
            strcat(buffer, conv);
            MPI_File_open(c, buffer, MPI_MODE_WRONLY | MPI_MODE_CREATE, MPI_INFO_NULL, &files[1][II]);
        }
        ...
        for (int II = 0; II < num_slices[2]; II++) {
            ...
            conv[0] = '\0';
            strcat(buffer, varname);
            sprintf(conv, "_slice_xy_%d.bin", II);
            strcat(buffer, conv);
            MPI_File_open(c, buffer, MPI_MODE_WRONLY | MPI_MODE_CREATE, MPI_INFO_NULL, &files[2][II]);
        }
    }

    void write_slices_2(DTArray & var, MPI_File ** files,
            double ** slice_coords, int * num_slices,
            int Nx, int Ny, int Nz, int slice_count, MPI_Comm c) {
        ...
        // yz slices
        offset = Nx*Ny*slice_count;
        double *yz_data_buffer = new double[Ny*Nz];
        for (II = 0; II < num_slices[0]; II++) {
            Ix = (int)(Nx*slice_coords[0][II]);

            if (Ix >= var.lbound(firstDim) && Ix <= var.ubound(firstDim)) {
                cnt = 0;
                for (Iy = 0; Iy < Ny; Iy++){
                    for (Iz = 0; Iz < Nz; Iz++) {
                        yz_data_buffer[cnt] = var(Ix,Iy,Iz);
                        cnt++;
                    }
                }

                MPI_File_seek(files[0][II], offset*sizeof(double), MPI_SEEK_SET);
                MPI_File_write(files[0][II], yz_data_buffer, Ny*Nz, MPI_DOUBLE, &status);
            }
        }
        delete[] yz_data_buffer;

        // xz slices
        offset = Nx*Nz*slice_count + Nx*Nz/numproc*myrank;
        double *xz_data_buffer = new double[Nz*Nx/numproc];
        for (II = 0; II < num_slices[1]; II++) {
            Iy = (int)(Ny*slice_coords[1][II]);

            cnt = 0;
            for (Ix = var.lbound(firstDim); Ix <= var.ubound(firstDim); Ix++) {
                for (Iz = 0; Iz < Nz; Iz++) {
                    xz_data_buffer[cnt] = var(Ix,Iy,Iz);
                    cnt++;
                }
            } 

            MPI_File_seek(files[1][II], offset*sizeof(double), MPI_SEEK_SET);
            MPI_File_write(files[1][II], xz_data_buffer, Nz*Nx/numproc, MPI_DOUBLE, &status);
        }
        delete[] xz_data_buffer;

        // xy slices
        offset = Nx*Ny*slice_count + Nx*Ny/numproc*myrank;
        double *xy_data_buffer = new double[Ny*Nx/numproc];
        for (II = 0; II < num_slices[2]; II++) {
            Iz = (int)(Nz*slice_coords[2][II]);

            cnt = 0;
            for (Ix = var.lbound(firstDim); Ix <= var.ubound(firstDim); Ix++) {
                for (Iy = 0; Iy < Ny; Iy++) {
                    xy_data_buffer[cnt] = var(Ix,Iy,Iz);
                    cnt++;
                }
            } 

            MPI_File_seek(files[2][II], offset*sizeof(double), MPI_SEEK_SET);
            MPI_File_write(files[2][II], xy_data_buffer, Ny*Nx/numproc, MPI_DOUBLE, &status);
        }
    ...
    void stitch_chains_2(char* varname, MPI_File ** files_from, MPI_File ** files_to, int * num_chains,
            ...
            int prev_chain_write, int lb, int ub, MPI_Comm c) {

        int myrank, numproc, offset_to, offset_from, Ix;
        MPI_Status status;
        ...
        // Stitch x chains
        double *chain_data_buffer = new double[Nx/numproc]; 
        for (int II = 0; II < num_chains[0]; II++) {
            for (int JJ = 0; JJ < chain_write_count; JJ++) {
                offset_from = JJ*Nx + Nx/numproc*myrank;
                offset_to   = (JJ + prev_chain_write)*Nx + Nx/numproc*myrank;

                MPI_File_seek(files_from[0][II], offset_from*sizeof(double), MPI_SEEK_SET);
                MPI_File_read(files_from[0][II], chain_data_buffer, Nx/numproc, MPI_DOUBLE, &status);
                MPI_File_seek(files_to[0][II], offset_to*sizeof(double), MPI_SEEK_SET);
                MPI_File_write(files_to[0][II], chain_data_buffer, Nx/numproc, MPI_DOUBLE, &status);
            }
        }
        delete[] chain_data_buffer;

        // Stitch y chains
        chain_data_buffer = new double[Ny]; 
        for (int II = 0; II < num_chains[1]; II++) {
            Ix = (int)(Nx*chain_coords[1][0][II]);
            if (Ix >= lb && Ix <= ub) {
                for (int JJ = 0; JJ < chain_write_count; JJ++) {
                    offset_from = JJ*Ny;
                    offset_to   = (JJ + prev_chain_write)*Ny;

                    MPI_File_seek(files_from[1][II], offset_from*sizeof(double), MPI_SEEK_SET);
                    MPI_File_read(files_from[1][II], chain_data_buffer, Ny, MPI_DOUBLE, &status);
                    MPI_File_seek(files_to[1][II], offset_to*sizeof(double), MPI_SEEK_SET);
                    MPI_File_write(files_to[1][II], chain_data_buffer, Ny, MPI_DOUBLE, &status);
                }
            }
        }
        delete[] chain_data_buffer;
        ...
        // Stitch z chains
        chain_data_buffer = new double[Nz]; 
        for (int II = 0; II < num_chains[2]; II++) {
            Ix = (int)(Nx*chain_coords[2][0][II]);
            if (Ix >= lb && Ix <= ub) {
                for (int JJ = 0; JJ < chain_write_count; JJ++) {
                    offset_from = JJ*Nz;
                    offset_to   = (JJ + prev_chain_write)*Nz;

                    MPI_File_seek(files_from[2][II], offset_from*sizeof(double), MPI_SEEK_SET);
                    MPI_File_read(files_from[2][II], chain_data_buffer, Nz, MPI_DOUBLE, &status);
                    MPI_File_seek(files_to[2][II], offset_to*sizeof(double), MPI_SEEK_SET);
                    MPI_File_write(files_to[2][II], chain_data_buffer, Nz, MPI_DOUBLE, &status);
                }
            }
        }
        delete[] chain_data_buffer;
        ...
        // Because of the way the were defined, this will also delete the files.
        MPI_Barrier(c);
        for (int II = 0; II < 3; II++) {
            for (int JJ = 0; JJ < num_chains[II]; JJ++) {
                MPI_File_close(&files_from[II][JJ]);
            }
        }
        MPI_Barrier(c);
    ...
    void stitch_slices_2(char* varname, MPI_File ** files_from, MPI_File ** files_to, int * num_slices,
            ...
            int prev_slice_write, int lb, int ub, MPI_Comm c) {
        ...
        // Stitch yz chains
        double *slice_data_buffer = new double[Ny*Nz];
        for (int II = 0; II < num_slices[0]; II++) {
            Ix = (int)(Nx*slice_coords[0][II]);
            if (Ix >= lb && Ix <= ub) {
                for (int JJ = 0; JJ < slice_write_count; JJ++) {
                    offset_from = JJ*Ny*Nz;
                    offset_to   = (JJ + prev_slice_write)*Ny*Nz;

                    MPI_File_seek(files_from[0][II], offset_from*sizeof(double), MPI_SEEK_SET);
                    MPI_File_read(files_from[0][II], slice_data_buffer, Ny*Nz, MPI_DOUBLE, &status);
                    MPI_File_seek(files_to[0][II], offset_to*sizeof(double), MPI_SEEK_SET);
                    MPI_File_write(files_to[0][II], slice_data_buffer, Ny*Nz, MPI_DOUBLE, &status);
                }
            }
        }
        delete[] slice_data_buffer;

        // Stitch xz chains
        slice_data_buffer = new double[Nx*Nz/numproc];
        for (int II = 0; II < num_slices[1]; II++) {
            for (int JJ = 0; JJ < slice_write_count; JJ++) {
                offset_from = JJ*Nx*Nz + Nz*Nx/numproc*myrank;
                offset_to   = (JJ + prev_slice_write)*Nx*Nz + Nz*Nx/numproc*myrank;

                MPI_File_seek(files_from[1][II], offset_from*sizeof(double), MPI_SEEK_SET);
                MPI_File_read(files_from[1][II], slice_data_buffer, Nx*Nz/numproc, MPI_DOUBLE, &status);
                MPI_File_seek(files_to[1][II], offset_to*sizeof(double), MPI_SEEK_SET);
                MPI_File_write(files_to[1][II], slice_data_buffer, Nx*Nz/numproc, MPI_DOUBLE, &status);
            }
        }
        delete[] slice_data_buffer;

        // Stitch xy chains
        slice_data_buffer = new double[Nx*Ny/numproc];
        for (int II = 0; II < num_slices[2]; II++) {
            for (int JJ = 0; JJ < slice_write_count; JJ++) {
                offset_from = JJ*Nx*Ny + Ny*Nx/numproc*myrank;
                offset_to   = (JJ + prev_slice_write)*Nx*Ny + Ny*Nx/numproc*myrank;

                MPI_File_seek(files_from[2][II], offset_from*sizeof(double), MPI_SEEK_SET);
                MPI_File_read(files_from[2][II], slice_data_buffer, Nx*Ny/numproc, MPI_DOUBLE, &status);
                MPI_File_seek(files_to[2][II], offset_to*sizeof(double), MPI_SEEK_SET);
                MPI_File_write(files_to[2][II], slice_data_buffer, Nx*Ny/numproc, MPI_DOUBLE, &status);
            }
        }
        delete[] slice_data_buffer;
