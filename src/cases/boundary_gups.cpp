...
//                   pow(mean(w(ii,kk,jj),kk),2))))*(length_y());
      ...
      void init_tracer(int t_num, DTArray & rho) {
         ...
         if (master()) fprintf(stderr,"Reading input rho\n");
         read_2d_slice(rho,"input_rho",2048,384);
      ...
      void init_vels(DTArray & u, DTArray & v, DTArray & w) {
         ...
         if (master()) fprintf(stderr,"Reading eta for velocity calculations\n");
         DTArray eta(alloc_lbound(szx,szy,szz),
               alloc_extent(szx,szy,szz),
               alloc_storage(szx,szy,szz));

         read_2d_slice(eta,"input_eta",2048,384);
         
         if (master()) fprintf(stderr,"Reading input phase speed\n");
         FILE * phase = fopen("input_c","r");
         assert(phase);
         fscanf(phase,"%lf",&PHASE_SPEED);
         if (master()) fprintf(stderr,"Got phase speed of %f\n",PHASE_SPEED);
         assert(PHASE_SPEED);
         fclose(phase);
         ...
         vorticity(fake_u, eta, fake_w, vort_u, vort_v, vort_w,
