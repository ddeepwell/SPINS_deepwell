...
#define H1 0.2
#define DELTA_RHO 0.1
#define PIC_WIDTH 0.025
#define G 9.81
#define SHEAR_U (-PHASE_SPEED)
#define BUMP_HEIGHT 0.25
...
#define PHASE_SPEED sqrt(DELTA_RHO*G*H1*(L_Z-H1)/L_Z)*.95
...
//                   pow(mean(w(ii,kk,jj),kk),2))))*(length_y());
      ...
      void init_tracer(int t_num, DTArray & rho) {
         ...
         Array<double,1> bump(split_range(szx));
         Array<double,2> eta(split_range(szx),blitz::Range(0,size_z()-1));
         /* The bump is sech^2 form */
         bump = BUMP_HEIGHT*pow(cosh(xx(ii)/BUMP_WIDTH),-2);
         /* Isopycnal displacement eta is trickier, since it must go to 0 at
            the boundaries.  In two-layer flow, it is C^0 and is linear.
            So, approximate that with two linear functions switched via tanh */
         eta = ((zz(jj)/(H1+bump(ii))) + ((zz(jj)-L_Z)/(H1-L_Z+bump(ii))-(zz(jj)/(H1+bump(ii))))/2*
                           (1+tanh((zz(jj)-H1-bump(ii))/PIC_WIDTH)))*bump(ii);
         rho = eta(ii,kk);
         write_array(rho,"eta_output"); write_reader(rho,"eta_output",false);
         rho = -DELTA_RHO/2*(tanh((zz(kk)-H1-eta(ii,kk))/PIC_WIDTH));
      ...
      void init_vels(DTArray & u, DTArray & v, DTArray & w) {
         ...
         /* Alright.  The right way of computing initial velocities here
            is to use eta as a streamfunction and then cross-differentiate
            to get velocities.  The expression for eta is extremely
            complicated, and I don't trust myself to take the derivative on
            paper.  So, I'm going to repurpose the vorticity code.

            If eta is "v", then x-vorticity is -v_z and y-vorticity is v_x.
            This is exactly the velocity we need, modulo a negative */
         DTArray fake_u(alloc_lbound(szx,szy,szz),
               alloc_extent(szx,szy,szz),
               alloc_storage(szx,szy,szz));
         DTArray fake_v(alloc_lbound(szx,szy,szz),
               alloc_extent(szx,szy,szz),
               alloc_storage(szx,szy,szz));
         DTArray fake_w(alloc_lbound(szx,szy,szz),
               alloc_extent(szx,szy,szz),
               alloc_storage(szx,szy,szz));
         fake_u = 0; fake_v = 0; fake_w = 0;
         fake_v = eta(ii,kk);
         DTArray * vort_u, * vort_v, * vort_w;
         vorticity(fake_u, fake_v, fake_w, vort_u, vort_v, vort_w,
               length_x(), length_y(), length_z(),
               size_x(), size_y(), size_z(),
               type_x(), type_y(), type_z());
         /* Now, scale by the phase speed */
         u = -(*vort_u) * PHASE_SPEED;
         w = -(*vort_w) * PHASE_SPEED;
         ...
         for (int i = u.lbound(firstDim); i<= u.ubound(firstDim); i++) {
            for (int j = u.lbound(secondDim); j<= u.ubound(secondDim); j++) {
               for (int k = u.lbound(thirdDim); k<= u.ubound(thirdDim); k++) {
                  u(i,j,k) += NOISE_MAG*rnd.random()+SHEAR_U;
      ...
      userControl(int s1, int s2, int s3):
         ...
         plot_interval(.025), nextplot(plot_interval),
