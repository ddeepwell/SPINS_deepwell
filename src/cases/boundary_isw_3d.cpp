...
class userControl : public BaseCase {
   public:
      ...
      double get_visco() const {
         return 1.0/100000; // 1e-5
      }
      double get_diffusivity(int t) const {
         return 1.0/100000/7; // 1e-5 / 7, as a thermocline
      }
...
//                   pow(mean(w(ii,kk,jj),kk),2))))*(length_y());
      ...
      userControl(int s1, int s2, int s3):
         ...
         plot_interval(.1), nextplot(plot_interval),
...
int main(int argc, char ** argv) {
   ...
   userControl mycode(4096,96,768);
