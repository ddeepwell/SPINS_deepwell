...
double kappa_dye;           // diffusivity of dye (m^2/s)
// helpful constants
const int NUM_TRACER = 2;
const int RHO = 0;          // index for rho
const int DYE = 1;          // index for dye
...
class dambreak : public BaseCase {
    public:
        ...
        void init_tracers(vector<DTArray *> & tracers) {
            ...
            DTArray & dye = *tracers[DYE];
            assert(numtracers() == int(tracers.size()));

            if (restarting and !restart_from_dump) {
                init_tracer_restart("rho",rho);
                init_tracer_restart("dye",dye);
            } else if (restarting and restart_from_dump) {
                init_tracer_dump("rho",rho);
                init_tracer_dump("dye",dye);
            } else {
                // density
                rho=0.2*exp(-pow(xx(ii)/1,2));
                rho  = -0.25*delta_rho*tanh((zz(kk)-0.05-rho)/0.01);
                rho += -0.25*delta_rho*tanh((zz(kk)-0.01+0.5*(1.0+tanh((xx(ii)-0.5*Lx)/0.25)))/0.01);
                // dye
                dye =  tanh((zz(kk)-0.025+0.5*(1.0+tanh((xx(ii)-0.5*Lx)/0.25)))/0.002);

                // Write the arrays
                write_array(rho,"rho",plotnum); 
                write_array(dye,"dye",plotnum); 
            }
        }
        ...
        void analysis(double time, DTArray & u, DTArray & v, DTArray & w,
            ...
            max_dye = psmax(max(abs(*tracers[DYE])));
            ...
            if ((time - next_plot) > -1e-6*plot_interval) {
                ...
                write_array(*tracers[DYE],"dye",plotnum);
                if (write_pressure)
        ...
        void write_variables(DTArray & u,DTArray & v, DTArray & w,
            ...
            write_array(*tracers[DYE],"dye.dump",-1);
        }
...
int main(int argc, char ** argv) {
    ...
    add_option("kappa_dye",&kappa_dye,0.0,"Diffusivity of density");	
