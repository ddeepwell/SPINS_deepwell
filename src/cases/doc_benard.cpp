...
const double alpha = 2.5e-4; // Thermal expansion of water, in units of kg / L / K
const double kappa = 1.3e-7; // Thermal diffusivity in water
const double nu = 1e-6; // Viscosity of water (divided by rho)

// Pysical parameters
const double LENGTH_X = 0.16; // 8cm side
const double LENGTH_Z = 0.02; // 2cm depth
const double DELTA_T = 2.0; // Temperature difference, bottom to top

// Computed, the Rayleigh number
const double RAYLEIGH = g*alpha*DELTA_T*LENGTH_Z*LENGTH_Z*LENGTH_Z/(kappa*nu);
// And the (negative) buoyancy frequency squared
const double N2 = g/rho_0*(alpha*DELTA_T/LENGTH_Z);
      ...
      void init_tracer(int t_num, DTArray & tprime) {
         ...
         for (int i = tprime.lbound(firstDim); i <= tprime.ubound(firstDim); i++) {
            ...
            for (int j = tprime.lbound(thirdDim); j <= tprime.ubound(thirdDim); j++) {
               ...
               // and cell area.
               tprime(i,0,j) = rnd.random()*DELTA_T*1e-3;
            }
         }
...
int main(int argc, char ** argv) {
   ...
   fprintf(stderr,"Rayleigh number %g, N2 %g\n",RAYLEIGH,N2);
