...
class userControl : public BaseCase {
   public:
      ...
      void do_mapping(DTArray & xg, DTArray & yg, DTArray & zg) {
         ...
         xgrid = alloc_array(szx,1,szy);
         zgrid = alloc_array(szx,1,szy);
         Array<double,1> xx(split_range(szx)), zz(szy);
         // Define numerical Cheby coordinates
         xx = -Lx*cos(ii*M_PI/(szx-1))/2;
         zz = -Ly*cos(ii*M_PI/(szy-1))/2;

         // Map only z for now, so x is normal
         xg = xx(ii)+0*kk;
//         xg = asin(.95*xx(ii))/asin(.95) + 0*kk;
         *xgrid = xg;

         yg = 0;

         zg = zz(kk) + (0.3)*xx(ii)*(Ly/2-zz(kk))/Ly;
//         zg = asin(.95*zz(kk))/asin(.95);
         *zgrid = zg;
...
/*         xx = -Lx/2 + Lx*(ii+0.5)/szx;
         ...
         /* For repeatability in parallel, seed the interface with
            the row number, rather than MPI rank.  This means that multi-
            processor runs (for a given grid size) will have the same
            exact ICs */
#if 0
         ...
         heat = delta_h*(0.5 + 
               0.5 * tanh(((*zgrid)(ii,jj,kk)-height-interface(ii))/thickness)) + 0*kk;
         salt = delta_s*(0.5 +
               0.5 * tanh(((*zgrid)(ii,jj,kk)-height)/thickness)) + 0*kk;
