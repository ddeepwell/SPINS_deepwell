   ...
   public:
      ...
      void analysis(double time, DTArray & u, DTArray & v, DTArray & w,
         ...
         /* Write out velocities and density if we've passed the designated
            plot time */
//         assert (!any(blitz_isnan(u)));
//         assert (!any(blitz_isnan(v)));
//         assert (!any(blitz_isnan(w)));
//         assert (!any(blitz_isnan(*tracer[0])));
//         assert (!any(blitz_isnan(*tracer[1])));
         bool plotted = false;
...
//         rnd.seed(myrank);
         for (int i = interface.lbound(firstDim); 
           ...
           interface(i) = thickness*1e-6*((i%5)-2);
         } 
...
//         assert (!any(blitz_isnan(*tracers[0])));
//         assert (!any(blitz_isnan(*tracers[1])));
         DTArray & heat = *tracers[HEAT];
         DTArray & salt = *tracers[SALT];
         u_f = 0; v_f = 0;
         w_f = -g*(rho_0 + rho_salt*salt - rho_h*heat);
//         assert (!any(blitz_isnan(u_f)));
//         assert (!any(blitz_isnan(v_f)));
//         assert (!any(blitz_isnan(w_f)));
      }
...
//         assert (!any(blitz_isnan(w)));
      ...
      userControl(int sz) : 
         szx(sz), szy(sz), Lx(0.1*sz/2048), Ly(0.1*sz/2048),
         plotnum(0), plot_interval(.5), nextplot(plot_interval),
         itercount(0),
         delta_h(5), delta_s(.04), height(0), thickness(1e-3) {
         }
};
...
int main(int argc, char ** argv) {
   ...
   if (npoints <= 0)
      npoints = 64;
   if (master()) fprintf(stdout,"%d points\n",npoints);
   userControl mycode(npoints);
   ...
   if (master())
      fprintf(stdout,"Total runtime: %fsec\n",later-now);
   MPI_Finalize();
   return 0;
}
