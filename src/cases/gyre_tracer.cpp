...
//               write_reader(tracer,"t2_output",true);
   ...
   void init_vels(DTArray & u, DTArray & v, DTArray & w) {
      ...
      u = sign*norm*wavenum_y*cos(wavenum_x*xx(ii))*cos(wavenum_y*yy(jj)) + 0*kk;
      fprintf(stderr,"max_u: %.2f\n",max(u));
      fprintf(stderr,"Initializing v\n");
      v = sign*norm*wavenum_x*sin(wavenum_x*xx(ii))*sin(wavenum_y*yy(jj)) + 0*kk;
      if (!initialized) {
...
//            write_array(*tracer[2],"t2_output",plotnum);
   ...
   userControl(): szx(64), szy(64), szz(1), 
                  Lx(2), Ly(2), Lz(1), 
                  plottime(0.25), plot_interval(0.25), plotnum(0),
                  mode_x(1), mode_y(1),
