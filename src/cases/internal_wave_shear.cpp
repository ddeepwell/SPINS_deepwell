...
/* These waves are defined on a geophysical scale, with lengths
   on the order of 500 meters.  These waves are centered in a 1km
   domain with fluid depth 100m.  While I'd love to be able to
   resolve everything, down to the viscous scales, the grid-scale
   Reynolds number is only O(1) if the grid is millimeter-scale.

   [Pf: Look at density, which changes at 1e-3*rho_o.  If that
   entire change happens over one grid cell, that gives a grid-
   scale density-Reynolds number of (1e-3*\Delta x)/1e-6.  Order
   unity requires \Delta x = O(1e-3).]

   Resolving down to the viscous scale (for physical parameter
   values) is impossible here, so pick a reasonable resolution
   (\Delta x = O(15cm)) and rely on filtering. */
      ...
      void forcing(double t, const DTArray & u, DTArray & u_f,
         ...
         w_f = -G*((*tracers[0])(ii,jj,kk)-base_rho(kk)) +
               AMPLITUDE*(1+0.1*phase_y(jj))*sin(FORCING_FREQ*t)*
                  ...
                  pow(cosh((zz(kk)-FORCING_LOC)/4),-2);
