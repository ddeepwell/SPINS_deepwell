...
double z_pyc;               // depth of pycnocline centre (m)
double z_mix;               // depth of pycnocline centre in collapse region (m)
double h_halfwidth;         // pycnocline half-width (m)
double h_mix_half;          // pycnocline half-width in collapse region (m)
// Horizontal stratification parameters
double delta_x;             // horizontal transition length (m)
double Lmix;                // Width of mixed region (m)
// dye parameters
double dye_halfwidth;
double dye_thickness;
    ...
    public:
        ...
        void init_tracers(vector<DTArray *> & tracers) {
            ...
            if (restarting and !restart_from_dump) {
                ...
                rho =  -0.5*delta_rho*tanh(((*zgrid)(ii,jj,kk)-z_pyc)/h_halfwidth);
                rho *=  0.5*(1.0+tanh(((*xgrid)(ii,jj,kk)-Lmix)/delta_x));
                // mixed region
                rho += 0.5*(1.0-tanh(((*xgrid)(ii,jj,kk)-Lmix)/delta_x))
                    *(-0.5)*delta_rho*tanh(((*zgrid)(ii,jj,kk)-z_mix)/h_mix_half);
...
int main(int argc, char ** argv) {
    ...
    add_option("z_pyc",&z_pyc,"pycnocline location");
    add_option("z_mix",&z_mix,"pycnocline location in collapse region");
    add_option("h_halfwidth",&h_halfwidth,"Pycnocline half-width");
    add_option("h_mix_half",&h_mix_half,"Pycnocline half-width in collapse region");
    add_option("delta_x",&delta_x,"Horizontal transition half-width");
    add_option("Lmix",&Lmix,"Width of mixed region");
    add_option("dye_thickness",&dye_thickness,"Thickness of the dye regions");
    add_option("dye_halfwidth",&dye_halfwidth,"Sharpness of the dye transition");
    ...
    // check if this is mapped - it should be
    if (!mapped) {
        if (master())
    ...
    /* ------------------ Do stuff --------------------- */
    mapiw mycode; // Create an instance of the above class
