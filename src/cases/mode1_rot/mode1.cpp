...
// Collapse stratification parameters
double Lmix;                // Width of mixed region (m)
double Hmix;                // Height of mixed region (m)
double Pmix;                // Power used in expression to create mixed region
    ...
    public:
        ...
        void init_tracers(vector<DTArray *> & tracers) {
            ...
            if (restarting and !restart_from_dump) {
                ...
                // stratification
                rho = -0.5*delta_rho*tanh((zz(kk)-z_pyc+Hmix*exp(-pow((xx(ii))/Lmix,Pmix)))/h_halfwidth);
                // Write the arrays
        ...
        void analysis(double time, DTArray & u, DTArray & v, DTArray & w,
            ...
            // Set-up
            if ( itercount == 1 ) {
                if (compute_dissipation or compute_stress) {
            ...
            double max_vort_x = psmax(max(abs(*temp1)));
            double enst_x_tot = pssum(sum(0.5*pow(*temp1,2)*
                        (*get_quad_x())(ii)*(*get_quad_y())(jj)*(*get_quad_z())(kk)));
            compute_vort_y(u, w, *temp1, gradient_op, grid_type);
            double max_vort_y = psmax(max(abs(*temp1)));
            double enst_y_tot = pssum(sum(0.5*pow(*temp1,2)*
                        (*get_quad_x())(ii)*(*get_quad_y())(jj)*(*get_quad_z())(kk)));
            compute_vort_z(u, v, *temp1, gradient_op, grid_type);
            double max_vort_z = psmax(max(abs(*temp1)));
            double enst_z_tot = pssum(sum(0.5*pow(*temp1,2)*
                        (*get_quad_x())(ii)*(*get_quad_y())(jj)*(*get_quad_z())(kk)));
            // max of fields
...
int main(int argc, char ** argv) {
    ...
    add_option("Pmix",&Pmix,"Power of mixed region");
