    ...
    public:
        ...
        void init_tracers(vector<DTArray *> & tracers) {
            ...
            if (restarting and !restart_from_dump) {
                ...
                rho =  -0.25*delta_rho*tanh(((*zgrid)(ii,jj,kk)-(MinZ+0.5*Lz+pyc_asym-0.5*pyc_sep))/h_halfwidth);
                rho += -0.25*delta_rho*tanh(((*zgrid)(ii,jj,kk)-(MinZ+0.5*Lz+pyc_asym+0.5*pyc_sep))/h_halfwidth);
                // second strat
                rho += delta_rho2*0.5*(1-tanh(((*zgrid)(ii,jj,kk)-z_pyc2)/h_pyc2));
...
int main(int argc, char ** argv) {
    ...
    add_option("dye_end",&dye_end,"");
    add_option("dye_xwidth",&dye_xwidth,"");
    add_option("delta_rho2",&delta_rho2,"density difference 2");
    add_option("z_pyc2",&z_pyc2,"");
    add_option("h_pyc2",&h_pyc2,"");
