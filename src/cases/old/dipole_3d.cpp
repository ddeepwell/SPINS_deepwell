...
class userControl : public BaseCase {
   public:
         ...
         if ((itercount - lastplot)%100 == 0 || plotted) {
            ...
            if (master())
               fprintf(stdout,"%f [%d] (%.4g, %.4g, %.4g) -- (%g, %g, %g)\n",
                     time, itercount, mu, mv, mw,enst,ke,ke-ke2d);
            if (master()) {
               FILE * en_record = fopen("energy_record.txt","a");
               assert(en_record);
               for (int i = 0; i < (itercount-last_writeout); i++) {
                  fprintf(en_record,"%g %.10g %.10g %.10g\n",times_record[i],
                        ke_record[i],enst_record[i],ke_record[i]-ke2d_record[i]);
               }
               fclose(en_record);
            }
      ...
      userControl(int s1, int s2):
         szx(s1), szy(s2), szz(s1+1),
         plotnum(0), itercount(0), lastplot(0), last_writeout(0),
         plot_interval(.005), nextplot(plot_interval) {
            compute_quadweights(szx,szy,szz,
                  length_x(),length_y(),length_z(),
                  type_x(),type_y(),type_z());
            if (master()) {
               printf("Using array size %dx%d\n",s1,s2);
            }
            if (restarting) {
               lastplot = restart_time;
