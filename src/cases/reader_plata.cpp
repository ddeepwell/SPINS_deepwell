...
//         if (master()) fprintf(stderr,"Input time %g\n",intime);
      ...
      void init_tracer(int t_num, DTArray & rho) {
         ...
         if (t_num == 0) {
            ...
            if (!threedeevel) {
               if (master()) fprintf(stderr,"Reading tracer (%d x %d) from %s\n",Nx,Nz,tracer_filename);
               read_2d_slice(rho,tracer_filename,Nx,Nz); 
            } else {
               if (master()) fprintf(stderr,"Reading tracer (%d x %d x %d) from %s\n",Nx,Ny,Nz,tracer_filename);
               read_array(rho,tracer_filename,Nx,Ny,Nz);
            }
...
int main(int argc, char ** argv) {
      ...
      plot_interval */
   {
      ...
      // Field filenames
      unused = fscanf(config_file,"%s %s %s %s %s\n",u_filename,v_filename,w_filename,rho_filename,tracer_filename);
      if (master()) fprintf(stderr,"Input file names:\nu: %s\nv: %s\nw: %s\nrho: %s\ntracers: %s\n",u_filename,
            v_filename, w_filename, rho_filename,tracer_filename);
