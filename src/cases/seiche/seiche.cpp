    ...
    public:
        ...
        void init_tracers(vector<DTArray *> & tracers) {
            ...
            if (restarting and !restart_from_dump) {
                ...
                rho = eta_0*cos(M_PI*xx(ii)/Lx);
                rho =  -0.5*delta_rho*tanh((zz(kk)-(MinZ + z_0 + rho))/h_halfwidth);
                // Write the arrays
...
int main(int argc, char ** argv) {
    ...
    add_option("eta_0",&eta_0,"Pycnocline amplitude");
    add_option("z_0",&z_0,"Average pycnocline depth");
    ...
    if (master()) {
        ...
        fprintf(stdout,"eta_0 = %g\n",eta_0);
        fprintf(stdout,"z_0 = %g\n",z_0);
