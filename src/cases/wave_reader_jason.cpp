...
// Perturbation for 2/3D extension
...
//         if (master()) fprintf(stderr,"Input time %g\n",intime);
      ...
      void init_vels(DTArray & u, DTArray & v, DTArray & w) {
         ...
         if (!threedeevel) {
            DTArray twodee_file(alloc_lbound(Nx,1,Nz),alloc_extent(Nx,1,Nz),alloc_storage(Nx,1,Nz));
            if (master()) fprintf(stderr,"Reading u (%d x %d) from %s\n",Nx,Nz,u_filename);
            read_array(twodee_file,u_filename,Nx,1,Nz);
            for (int j = u.lbound(secondDim); j <= u.ubound(secondDim); j++) {
               u(blitz::Range::all(),blitz::Range(j,j),blitz::Range::all()) = 
                  twodee_file(blitz::Range::all(),blitz::Range(0,0),blitz::Range::all());
            }


//            read_2d_slice(u,u_filename,Nx,Nz);
            if (master()) fprintf(stderr,"Reading w (%d x %d) from %s\n",Nx,Nz,w_filename);
            read_array(twodee_file,w_filename,Nx,1,Nz);
            for (int j = u.lbound(secondDim); j <= u.ubound(secondDim); j++) {
               v(blitz::Range::all(),blitz::Range(j,j),blitz::Range::all()) = 
                  twodee_file(blitz::Range::all(),blitz::Range(0,0),blitz::Range::all());
            }
            if (Ny > 1 || rot_f != 0) {
               if (master()) fprintf(stderr,"Reading v (%d x %d) from %s\n",Nx,Nz,v_filename);
               read_array(twodee_file,v_filename,Nx,1,Nz);
               for (int j = u.lbound(secondDim); j <= u.ubound(secondDim); j++) {
                  v(blitz::Range::all(),blitz::Range(j,j),blitz::Range::all()) = 
                     twodee_file(blitz::Range::all(),blitz::Range(0,0),blitz::Range::all());
               }
            ...
            if (Ny > 1 && perturbation > 0) {
               Normal<double> rnd(0,1);
               for (int i = u.lbound(firstDim); i <= u.ubound(firstDim); i++) {
                  rnd.seed(i);
                  for (int j = u.lbound(secondDim); j <= u.ubound(secondDim); j++) 
                     for (int k = u.lbound(thirdDim); k <= u.ubound(thirdDim); k++) {
                        u(i,j,k) = u(i,j,k)*(1+perturbation*rnd.random());
                        w(i,j,k) = w(i,j,k)*(1+perturbation*rnd.random());
                        v(i,j,k) = v(i,j,k)*(1+perturbation*rnd.random());
                     }
               }
            }
      ...
      void init_tracer(int t_num, DTArray & rho) {
         ...
         if (!threedeevel) {
            ...
            read_array(twodee_file,rho_filename,Nx,1,Nz);
...
int main(int argc, char ** argv) {
      ...
      plot_interval */
   {
      ...
      if (master()) fprintf(stderr,"3D velocity perturbation = %g\n",perturbation);
