   ...
   public:
      ...
      double check_timestep(double intime, double now) {
         if (intime < 1e-9) {
            ...
            intime = .05; // And process to make sure we land on a plot time
         }
      ...
      void vel_forcing(double t, DTArray & u_f, DTArray & v_f, DTArray & w_f,
         ...
         u_f = 0; v_f = 0;
         w_f = -g*(*tracers[0]) +
               force_amplitude/cosh((zz(kk)-height)/force_zwidth)/
               cosh(xx(ii)/force_xwidth) * tanh((zz(kk)-height)/force_zwidth) *
               sin(2*M_PI*t/4);
      }
      ...
      userControl() : 
         szx(1024), szz(256), Lx(10), Lz(1),
         plotnum(0), plot_interval(0.2), nextplot(plot_interval),
         itercount(0), g(9.805e-3), 
         height(-0.3), pic_width(0.03), pic_strength(50),
         force_zwidth(.03), force_amplitude(0.25), force_xwidth(.15),
         zz(szz), xx(split_range(szx)) {
         }
};
extern template class FluidEvolve<userControl>;
int main() {
   MPI_Init(0,0);
   userControl mycode;
