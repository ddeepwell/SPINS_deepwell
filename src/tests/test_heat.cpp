...
/* FIXME: This test case is currently not working. */
...
int main(int argc, char * argv[]) {
   ...
   us.setp(-2,new DTArray(szx, szy, szz)); // Initialize members
   us.setp(-1,new DTArray(szx, szy, szz));
   us.setp( 0,new DTArray(szx, szy, szz));
   us.setp( 1,new DTArray(szx, szy, szz));
   
   DTArray rhs(szx, szy, szz); // rhs for implicit solve
   DTArray exact_soln(szx, szy, szz); // exact solution

   Array<double,1> xx(szx), yy(szy), zz(szz); // grid
...
//      exact_soln = exp(-mode*mode*times[1])*cos(xx(ii)*mode);
      double max_err = max(abs(us[1]-exact_soln))/max(exact_soln);
